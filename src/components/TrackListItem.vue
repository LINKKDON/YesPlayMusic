<template>
  <div
    ...
    :class="trackClass"
    :style="trackStyle"
    :title="showUnavailableSongInGreyStyle ? playableStatus.reason : ''"
...
export default {
  ...
  computed: {
    ...
    // 实时计算播放权限（读取 store 设置，切换第三方音源开关时灰化即时更新）
    playableStatus() {
      if (this.track?.privilege?.pl > 0) return { playable: true, reason: '' };
      if (!this.track) return { playable: false, reason: '' };
      return isTrackPlayable(this.track);
    },
    playable() {
      return this.playableStatus.playable;
    },
    imgUrl() {
      let image =
